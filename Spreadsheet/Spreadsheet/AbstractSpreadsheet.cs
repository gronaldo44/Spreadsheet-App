...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// Visit the cell being updated and each of its dependents in order of dependancy
        ///   
        /// For example, suppose that
        /// a1 contains 5
        /// b1 contains formula a1 + c1
        /// c1 contains formula a1 - 2
        /// d1 contains 10
        /// 
        /// If a1 changed, then the valaues of cells a1, b1, and c1 need to be changed,
        /// and they must be changed in an order which has a1 first, and c1 before b1.
        /// 
        /// <param name="start">The cell being updated</param>
        /// <param name="name">The cell currently being visited</param>
        /// <param name="visited">Set of all the cells already visited</param>
        /// <param name="changed">Ordered list of all the cells whos values need to be changed</param>
        /// </summary>
        private void Visit(string start, string name, ISet<string> visited, LinkedList<string> changed)
        {
            visited.Add(name);  // This cell is being visited
            foreach (string n in GetDirectDependents(name)) // Visit each of this cell's direct dependents
            {
                if (n.Equals(start))
                {   // The cell being updated is a dependent of itself
                    throw new CircularException();
                }
                else if (!visited.Contains(n))
                {   // This direct dependent has not been visited yet
                    Visit(start, n, visited, changed);  // Recursively visit the cell
                }
            }
            changed.AddFirst(name); // This cell's value should be the next one changed
        }
